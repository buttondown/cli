...
export function serialize(email: Partial<Email & FrontMatterFields>): string {
  ...
  const cleanedBody = body?.replace(EDITOR_MODE_PATTERN, "");
...
/*
  Replaces the relative image references in the content with the corresponding URLs from the syncedImages object.
*/
